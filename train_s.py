...
if __name__ == "__main__":
    ...
    model.train(
        ...
        cache=True,  # 将所有图片缓存到 RAM 中。极大提升训练速度，但极其消耗内存
        epochs=150,
        # imgsz=640,
        imgsz=[1536, 1920],
        ...
        profile=True,  # 是否分析每个步骤的时间和内存占用onnx或tesorrt
        # batch=-1,
        batch=16,
        pretrained=False,
        patience=10,
        ...
        mixup=0.02,  # 0.05 有 5% 的概率，系统会把两张完全不同的图片，像幻灯片一样设置一定的透明度（Alpha 通道）叠在一起
        cutmix=0.00082,
        close_mosaic=10,  # 在最后 10 个 Epoch 关闭 Mosaic 增强
        # copy_paste=0.404,# 40.4% 的概率触发复制粘贴增强
        copy_paste=0,  # 无Mask 掩码，不增强
        # scale=0.9,
        ...
        translate=0.275,
        # hsv_h=0.013,
        hsv_h=0.0,  # 禁止改变色相 (严禁红变绿)
        hsv_s=0.353,  # 允许饱和度变化 (模拟不同天气)
        hsv_v=0.194,  # 允许亮度变化 (模拟昼夜)
        bgr=0.0,
        # Loss Weights
        box=9.83,  # 损失的权重比例
        cls=0.65,
