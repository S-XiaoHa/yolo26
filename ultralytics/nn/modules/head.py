...
__all__ = "OBB", "Classify", "Detect", "Detect26", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class Detect26(Detect):
    """YOLO26 dual-task detection head: cv3(shape nc classes) + cv4(color ex_nc classes).

    This class extends the standard Detect head to support dual independent classification tasks:
    - Shape classification (nc classes)
    - Color classification (ex_nc classes)

    Attributes:
        nc (int): Number of shape classes.
        ex_nc (int): Number of color classes.
        cv4 (nn.ModuleList): Convolution layers for color classification.
        one2one_cv4 (nn.ModuleList): One-to-one convolution layers for color classification.

    Methods:
        forward_head: Perform forward pass and return predictions with scores_ex.
        bias_init: Initialize detection head biases including color branch.

    Examples:
        Create a dual-task detection head
        >>> detect26 = Detect26(nc=5, ex_nc=3, ch=(256, 512, 1024))
        >>> x = [torch.randn(1, 256, 80, 80), torch.randn(1, 512, 40, 40), torch.randn(1, 1024, 20, 20)]
        >>> outputs = detect26(x)
    """

    def __init__(self, nc: int = 5, ex_nc: int = 3, reg_max=16, end2end=False, ch: tuple = ()):
        """Initialize the YOLO26 dual-task detection layer.
        ...
        c4 = max(ch[0], min(self.ex_nc, 100))

        # cv4: color classification branch (mimics YOLO26's DWConv+Conv double-layer structure)
        self.cv4 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(DWConv(x, x, 3), Conv(x, c4, 1)),
                nn.Sequential(DWConv(c4, c4, 3), Conv(c4, c4, 1)),
                nn.Conv2d(c4, self.ex_nc, 1),
            )
            for x in ch
        )
        if end2end:
    ...
    @property
    def one2many(self):
        """Returns the one-to-many head components including color classification head."""
        return dict(box_head=self.cv2, cls_head=self.cv3, ex_cls_head=self.cv4)

    @property
    def one2one(self):
        """Returns the one-to-one head components including color classification head."""
        return dict(
            box_head=self.one2one_cv2,
            cls_head=self.one2one_cv3,
            ex_cls_head=self.one2one_cv4,
        )
    ...
    def forward_head(
        ...
        ex_cls_head: torch.nn.Module = None,
    ) -> dict[str, torch.Tensor]:
        """Concatenates and returns predicted bounding boxes, shape scores, and color scores."""
        if box_head is None or cls_head is None or ex_cls_head is None:
            return dict()
        preds = super().forward_head(x, box_head, cls_head)
        bs = x[0].shape[0]  # batch size
        preds["scores_ex"] = torch.cat(
            [ex_cls_head[i](x[i]).view(bs, self.ex_nc, -1) for i in range(self.nl)], dim=-1
        )
        return preds
    ...
    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        """Post-process predictions with combined ``nc * ex_nc`` class scores."""
        boxes, scores = preds.split([4, self.nc * self.ex_nc], dim=-1)
        scores, conf, idx = self.get_topk_index(scores, self.max_det)
        boxes = boxes.gather(dim=1, index=idx.repeat(1, 1, 4))
        return torch.cat([boxes, scores, conf], dim=-1)

    def bias_init(self):
        """Initialize Detect26 biases, including color branch biases."""
        super().bias_init()
        for i, c in enumerate(self.cv4):
            c[-1].bias.data[:] = math.log(5 / self.ex_nc / (640 / self.stride[i]) ** 2)
        if self.end2end:
            for i, c in enumerate(self.one2one_cv4):
                c[-1].bias.data[:] = math.log(5 / self.ex_nc / (640 / self.stride[i]) ** 2)

    def fuse(self) -> None:
        """Remove the one2many heads for inference optimization."""
        self.cv2 = self.cv3 = self.cv4 = None
