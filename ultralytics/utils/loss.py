...
class v8DetectionLoss26(v8DetectionLoss):
    """Dual-task detection loss for YOLO26: dynamically maps 15-class labels to shape + color dual losses.

    This class extends v8DetectionLoss to support dual independent classification tasks:
    - Shape classification (nc=5 classes) via the primary cls head
    - Color classification (ex_nc=3 classes) via the secondary ex_cls head

    The 15 original classes (5 shapes x 3 colors) are dynamically mapped:
    - shape_map: [0,0,0, 1,1,1, 2,2,2, 3,3,3, 4,4,4]  (5 shape classes: round,up,left,right,turn_around)
    - color_map: [0,1,2, 0,1,2, 0,1,2, 0,1,2, 0,1,2]  (3 color classes: red,yellow,green)

    Original 15 classes mapping:
      0:round_red, 1:round_yellow, 2:round_green,
      3:up_red, 4:up_yellow, 5:up_green,
      6:left_red, 7:left_yellow, 8:left_green,
      9:right_red, 10:right_yellow, 11:right_green,
      12:turn_around_red, 13:turn_around_yellow, 14:turn_around_green
    """

    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):
        """Initialize v8DetectionLoss26 with dual-task mapping tables.

        Args:
            model: The YOLO model (must be de-paralleled).
            tal_topk: Top-k parameter for TaskAlignedAssigner.
            tal_topk2: Secondary top-k parameter for TaskAlignedAssigner.
        """
        super().__init__(model, tal_topk, tal_topk2)
        m = model.model[-1]  # Detect26() module
        self.nc_shape = m.nc  # number of shape classes (5)
        self.ex_nc = m.ex_nc  # number of color classes (3)

        # 15 classes → shape(5 classes) + color(3 classes) mapping tables
        # Shape: 0=round, 1=up, 2=left, 3=right, 4=turn_around
        # Color: 0=red, 1=yellow, 2=green
        # NOTE: v8DetectionLoss is NOT an nn.Module, so use direct tensor assignment
        self.shape_map = torch.tensor(
        ...
        # Update assigner to use shape classes (not original 15 classes)
        self.assigner.num_classes = self.nc_shape

    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        """Calculate loss for dual-task detection: box + shape_cls + color_cls + dfl.

        Returns 3-element loss tensor (box, cls_combined, dfl) to maintain compatibility
        with the parent class interface, where cls_combined = shape_loss + color_loss.
        """
        loss = torch.zeros(3, device=self.device)  # box, cls(shape+color), dfl

        pred_distri = preds["boxes"].permute(0, 2, 1).contiguous()
        pred_scores_shape = preds["scores"].permute(0, 2, 1).contiguous()
        pred_scores_color = preds["scores_ex"].permute(0, 2, 1).contiguous()

        anchor_points, stride_tensor = make_anchors(preds["feats"], self.stride, 0.5)

        dtype = pred_scores_shape.dtype
        batch_size = pred_scores_shape.shape[0]
        imgsz = torch.tensor(preds["feats"][0].shape[2:], device=self.device, dtype=dtype) * self.stride[0]

        # === Extract and map ground truth labels ===
        targets = torch.cat((batch["batch_idx"].view(-1, 1), batch["cls"].view(-1, 1), batch["bboxes"]), 1)
        targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        gt_raw_labels, gt_bboxes = targets.split((1, 4), 2)
        mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0.0)

        # Dynamic label mapping: 15 raw classes -> shape (nc=5) + color (ex_nc=3)
        # NOTE: ``mask_gt`` is float (0.0 / 1.0) because the parent uses the in-place
        # ``gt_`` op, so cast to bool before using it as a boolean index.
        valid_mask = mask_gt.squeeze(-1).bool()
        gt_shape_labels = torch.zeros_like(gt_raw_labels)
        gt_color_labels = torch.zeros_like(gt_raw_labels)
        if valid_mask.any():
            raw = gt_raw_labels[valid_mask].long().view(-1)
            gt_shape_labels[valid_mask] = self.shape_map[raw].to(gt_shape_labels.dtype).view(-1, 1)
            gt_color_labels[valid_mask] = self.color_map[raw].to(gt_color_labels.dtype).view(-1, 1)

        # === Decode bounding boxes ===
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri)

        # === Assigner: use shape scores for positive/negative sample assignment ===
        _, target_bboxes, target_scores, fg_mask, target_gt_idx = self.assigner(
            pred_scores_shape.detach().sigmoid(),
            (pred_bboxes.detach() * stride_tensor).type(gt_bboxes.dtype),
            anchor_points * stride_tensor,
            gt_shape_labels,
            gt_bboxes,
        ...
        # === Shape CLS Loss ===
        loss_shape = self.bce(pred_scores_shape, target_scores.to(dtype)).sum() / target_scores_sum

        # === Color CLS Loss ===
        # Gather per-anchor color targets via the same global-index trick used by
        # TaskAlignedAssigner.get_targets: target_gt_idx (b, h*w) is per-batch,
        # so we must add batch_ind * n_max_boxes before indexing the flat gt tensor.
        loss_color = torch.zeros(1, device=self.device)[0]
        if fg_mask.sum() > 0:
            n_max_boxes = gt_color_labels.shape[1]
            batch_ind = torch.arange(
                end=batch_size, dtype=torch.int64, device=self.device
            )[..., None]
            target_color = gt_color_labels.long().flatten()[
                target_gt_idx + batch_ind * n_max_boxes
            ]  # (b, h*w)

            target_color_onehot = torch.zeros(
                (target_color.shape[0], target_color.shape[1], self.ex_nc),
                dtype=torch.int64,
                device=self.device,
            )
            target_color_onehot.scatter_(2, target_color.unsqueeze(-1).clamp_(0), 1)

            fg_scores_mask = fg_mask[:, :, None].repeat(1, 1, self.ex_nc)
            target_scores_color = torch.where(fg_scores_mask > 0, target_color_onehot.float(), 0)

            loss_color = self.bce(pred_scores_color, target_scores_color.to(dtype)).sum() / target_scores_sum

        # Combine shape + color loss into the cls_loss slot
        loss[1] = loss_shape + loss_color

        # === Bbox Loss (maintains native YOLO26 mechanism) ===
        if fg_mask.sum():
            loss[0], loss[2] = self.bbox_loss(
                pred_distri,
                pred_bboxes,
        ...
        # === Apply weight gains ===
        loss[0] *= self.hyp.box  # box gain
        loss[1] *= self.hyp.cls  # shape + color share cls weight
        loss[2] *= self.hyp.dfl  # dfl gain
        ...
        return (
            ...
            loss,
            loss.detach(),
        )  # loss(box, cls_combined=shape+color, dfl)
